...
export function DocsWorkspace({ initialData, developmentPreview = false }: { initialData: DocsPageData; developmentPreview?: boolean }) {
  ...
  return <div className="px-4 py-4 sm:px-5 lg:px-6 lg:py-5">
    ...
    <DocEditor key={draft?.id ?? "empty"} draft={draft} projects={workspace.projects} busy={busy} developmentPreview={developmentPreview} onClose={() => setDraft(null)} onChange={setDraft} onSave={() => void save()} onDelete={() => void remove()} />
  </div>;
}
