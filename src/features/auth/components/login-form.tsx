...
export function LoginForm() {
  ...
  return (
    <form className="space-y-4" method="POST" onSubmit={onSubmit} noValidate>
      ...
      {serverError ? <p className="rounded-lg border border-white/[0.08] bg-white/[0.035] px-3 py-2 text-[11px] leading-5 text-muted-foreground" role="alert">{serverError}</p> : null}
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-1.5 block text-xs font-medium text-foreground">{label}</span>{children}{error ? <span className="mt-1.5 block text-[11px] text-muted-foreground">{error}</span> : null}</label>;
}
